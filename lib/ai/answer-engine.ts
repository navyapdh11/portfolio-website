...
import { knowledgeBase, type FAQEntry, type ServiceCategory, type PolicyEntry } from "../data/knowledge-base";
...
const STOP_WORDS = new Set([
  ...
  "shall", "can", "need", "must", "have", "has", "had", "having",
  ...
  "of", "at", "by", "for", "with", "from", "to", "on", "in", "up", "out",
  "if", "then", "else", "because", "as", "until", "while",
  "am", "also", "more", "most", "some", "any", "all", "each", "every",
  "much", "many", "such", "no", "never", "always", "often", "sometimes",
  "here", "there", "again", "further", "once", "now",
  "do", "does", "did", "don't", "doesn't", "didn't",
  "i'm", "you're", "he's", "she's", "it's", "we're", "they're",
  "i've", "you've", "we've", "they've",
  "i'll", "you'll", "he'll", "she'll", "we'll", "they'll",
  "i'd", "you'd", "he'd", "she'd", "we'd", "they'd",
  "let", "make", "get", "go", "like", "want", "please", "tell", "say",
...
export function retrieve(query: string, topK: number = 5): ScoredResult[] {
  ...
  // Score booking steps
  const bookingText = knowledgeBase.bookingSteps.map((s) => `${s.title}: ${s.description}`).join(" ");
  const bookingScore = scoreText(queryTokens, bookingText, ["book", "booking", "schedule", "step", "process"]);
  if (bookingScore > 0) {
    results.push({
      score: bookingScore,
      source: "booking",
  ...
  const contactScore = scoreText(queryTokens, contactText, ["contact", "phone", "email", "call", "reach", "hours"]);
  if (contactScore > 0) {
    results.push({
      score: contactScore,
      source: "contact",
...
// ---------------------------------------------------------------------------
export function generateAnswer(query: string, results: ScoredResult[]): string {
  if (results.length === 0) {
...
export function getRelatedQuestions(query: string, results: ScoredResult[]): string[] {
  ...
  for (const result of results.slice(0, 3)) {
    ...
    if (result.source === "service") {
      ...
      if (!seen.has(pricingQ)) { suggestions.push(pricingQ); seen.add(pricingQ); }
      if (!seen.has(bookingQ)) { suggestions.push(bookingQ); seen.add(bookingQ); }
    }
    ...
    if (result.source === "area") {
      ...
      if (!seen.has(servicesQ)) { suggestions.push(servicesQ); seen.add(servicesQ); }
...
export const QUICK_ACTIONS: { label: string; icon: string; query: string }[] = [
  { label: "Get a Quote", icon: "💰", query: "How much do your services cost?" },
