...
export function csrfCheck(request: Request): { valid: boolean; error?: string; token?: string } {
  ...
  if (origin) {
    ...
    if (!isTrusted) {
      return { valid: false, error: `CSRF validation failed — untrusted Origin: ${origin}` };
    }
  }
 ...
 */
export function csrfResponse(request: Request): { response: NextResponse | null; cookie?: string } {
  const result = csrfCheck(request);
  if (!result.valid) {
    return { response: NextResponse.json({ error: result.error }, { status: 403 }) };
  }
