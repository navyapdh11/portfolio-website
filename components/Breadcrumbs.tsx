...
export default function Breadcrumbs() {
	...
	if (segments[0] === "services" && segments.length >= 4) {
		...
		breadcrumbs.push({
			label: service?.name || capitalize(serviceSlug),
			href: `/services/${serviceSlug}`,
		});
		breadcrumbs.push({
			label: `${capitalize(citySlug)}, ${stateName}`,
			href: pathname,
		});

		return (
			<nav
				className="flex items-center gap-1.5 text-sm text-zinc-500 dark:text-zinc-400 mb-6"
