  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setFormData({ name: "", email: "", phone: "", suburb: city, state, address: "", date: "", time: "09:00", frequency: "one-time", message: "" });
        setStep(1);
      } else {
    ...
    <section id="booking" className="bg-white dark:bg-zinc-900 rounded-3xl shadow-2xl p-8 border border-zinc-200 dark:border-zinc-800">
      ...
      {step === 1 && (
        <div className="space-y-6">
          ...
          <ul className="text-sm text-zinc-600 space-y-2">
            {details.included.map((inc, i) => <li key={i} className="flex items-center gap-2">✓ {inc}</li>)}
          </ul>
          <button onClick={() => setStep(2)} className="w-full py-4 bg-blue-600 text-white rounded-xl font-bold hover:bg-blue-700 transition-colors">
      ...
      {step === 2 && (
        <div className="space-y-4">
          ...
          <select value={formData.frequency} onChange={e => setFormData({...formData, frequency: e.target.value})} className="w-full p-3 border border-zinc-300 rounded-lg focus:ring-2 focus:ring-blue-500 focus:outline-none">
        ...
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-3">
            ...
            <div>
              ...
              <input type="text" value={formData.state} readOnly className="w-full p-3 border border-zinc-300 rounded-lg bg-zinc-100" />
            </div>
          </div>
      ...
      {step === 4 && (
        <div className="space-y-4">
          ...
          <div className="bg-zinc-50 dark:bg-zinc-800 rounded-xl p-4 space-y-2 text-sm">
            <div className="flex justify-between"><span className="text-zinc-500">Service</span><span className="font-medium">{serviceSlug.replace(/-/g, ' ')}</span></div>
            <div className="flex justify-between"><span className="text-zinc-500">Date</span><span className="font-medium">{formData.date}</span></div>
            <div className="flex justify-between"><span className="text-zinc-500">Time</span><span className="font-medium">{formData.time}</span></div>
            <div className="flex justify-between"><span className="text-zinc-500">Frequency</span><span className="font-medium capitalize">{formData.frequency}</span></div>
            <div className="flex justify-between"><span className="text-zinc-500">Address</span><span className="font-medium">{formData.address}, {formData.suburb}</span></div>
          </div>
          <button onClick={handleSubmit} disabled={isSubmitting} className="w-full py-4 bg-green-600 text-white rounded-xl font-bold hover:bg-green-700 transition-colors disabled:opacity-50 disabled:cursor-not-allowed">
            {isSubmitting ? "Submitting..." : "Confirm Booking"}
          </button>
          <p className="text-center text-xs text-zinc-500">We&apos;ll send you a quote after confirming your requirements. No upfront payment required.</p>
        </div>
      )}
    </section>
  );
