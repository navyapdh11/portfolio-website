...
{[
            { label: "Page Views", value: formatNumber(metrics.pageViews.current), change: 12.5, icon: "👁️" },
            { label: "Visitors", value: formatNumber(metrics.uniqueVisitors.current), change: 8.2, icon: "👤" },
            { label: "Session Duration", value: "2:34", change: 5.1, icon: "⏱️" },
            ...
            <div key={metric.label} className="bg-white dark:bg-zinc-800 p-4 rounded-xl shadow-md">
              ...
              <p className={`text-xs ${metric.change > 0 ? "text-green-600" : "text-red-600"}`}>
                {metric.change > 0 ? "↑" : "↓"} {Math.abs(typeof metric.change === "number" ? metric.change : 0).toFixed(1)}%
              </p>
            </div>
          ))}
        </div>
          ...
          <div className="p-6">
              ...
              <div className="space-y-6">
                <div className="grid md:grid-cols-2 gap-6">
                  ...
                  <div className="p-4 bg-zinc-50 dark:bg-zinc-700 rounded-xl">
                    ...
                    <div className="space-y-2">
                      {topPages.map((page) => (
                        <div key={page.page} className="flex justify-between items-center p-2 bg-white dark:bg-zinc-800 rounded-lg">
                          <span className="text-sm">{page.page}</span>
                          <div className="text-right">
                            <span className="text-sm font-medium">{page.views}</span>
                            <span className="text-xs text-zinc-500 ml-2">({page.bounce}% bounce)</span>
                          </div>
                        </div>
                      ))}
                    </div>
                ...
                <div className="p-4 bg-slate-100 dark:bg-slate-700 rounded-xl">
                  ...
                  <div className="h-40 flex items-end gap-1">
                    {[40, 55, 45, 60, 75, 65, 80, 70, 85, 90, 78, 95, 88, 100, 92, 85, 95, 105, 98, 110, 102, 115, 108, 120, 112, 125, 118, 130, 122, 135].map((val, i) => (
                      <div key={i} className="flex-1 bg-slate-400 rounded-t" style={{ height: `${val}%` }}></div>
                    ))}
                  </div>
                </div>
              </div>
                  ...
                  <div className="overflow-x-auto">
                    <table className="w-full">
                      ...
                      <tbody className="text-sm">
                        <tr><td>Organic traffic</td><td>4.2K</td><td>8.5K</td><td>6.2K</td><td>4.8K</td></tr>
                        <tr><td>Page 1 keywords</td><td>12</td><td>28</td><td>22</td><td>15</td></tr>
                        <tr><td>Backlinks</td><td>156</td><td>420</td><td>310</td><td>245</td></tr>
                        <tr><td>Domain authority</td><td>24</td><td>42</td><td>38</td><td>31</td></tr>
                      </tbody>
                    </table>
                  </div>
                </div>
            ...
            {activeTab === "cro" && (
              <div className="space-y-6">
                <div className="grid md:grid-cols-3 gap-4">
                  {[
                    { label: "Landing Page CVR", value: croMetrics.landingPageConversion + "%", color: "text-green-600" },
                    { label: "CTA Click Rate", value: croMetrics.ctaClickRate + "%", color: "text-blue-600" },
                    { label: "Form Completion", value: croMetrics.formCompletionRate + "%", color: "text-purple-600" },
                  ].map((m) => (
                    <div key={m.label} className="p-4 bg-zinc-50 dark:bg-zinc-700 rounded-xl text-center">
                ...
                <div className="p-4 bg-zinc-50 dark:bg-zinc-700 rounded-xl">
                  ...
                  <div className="space-y-3">
                    {[
                      { priority: "high", rec: "Add urgency messaging to CTA buttons", impact: "+12%" },
                      { priority: "high", rec: "Reduce form fields from 5 to 3", impact: "+25%" },
                      { priority: "medium", rec: "Add trust badges near booking form", impact: "+8%" },
                      { priority: "low", rec: "Fix mobile checkout flow", impact: "+5%" },
                    ].map((r, i) => (
                      <div key={i} className="flex justify-between items-center p-3 bg-white dark:bg-zinc-800 rounded-lg">
                        <div className="flex items-center gap-3">
                          <span className={`w-2 h-2 rounded-full ${
                            r.priority === "high" ? "bg-red-500" : r.priority === "medium" ? "bg-amber-500" : "bg-green-500"
                          }`}></span>
                          <span>{r.rec}</span>
                        </div>
