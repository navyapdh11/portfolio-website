...
export default function MicrotasksContent() {
  ...
  const tasks: Task[] = [
    { id: 1, prompt: 'Photograph 5 dirty kitchen counters before cleaning', type: 'image', reward: 0.75, estTime: '45s', rarity: 'common', category: 'kitchen' },
    { id: 2, prompt: 'Record instructions: "How to clean marble countertops" in Spanish', type: 'audio', reward: 1.25, estTime: '30s', rarity: 'rare', category: 'surface' },
    { id: 3, prompt: 'Scan handwritten cleaning checklist (English)', type: 'image', reward: 0.50, estTime: '25s', rarity: 'common', category: 'documentation' },
    { id: 4, prompt: 'Label stains: Upload bathroom mirror photo + types', type: 'annotation', reward: 1.00, estTime: '60s', rarity: 'medium', category: 'bathroom' },
    { id: 5, prompt: 'Before/after: Vacuum marks on carpet', type: 'image-pair', reward: 1.50, estTime: '90s', rarity: 'high', category: 'flooring' },
    { id: 6, prompt: 'Record "Stain removal techniques" in Mandarin', type: 'audio', reward: 1.75, estTime: '40s', rarity: 'rare', category: 'stain' },
    { id: 7, prompt: 'Photograph office desk organization stages', type: 'image-sequence', reward: 1.20, estTime: '75s', rarity: 'medium', category: 'office' },
    { id: 8, prompt: 'Annotate cleaning product labels (5 items)', type: 'text', reward: 0.85, estTime: '50s', rarity: 'common', category: 'products' },
    { id: 9, prompt: 'Capture move-out apartment conditions', type: 'image', reward: 1.00, estTime: '60s', rarity: 'medium', category: 'moveout' },
    { id: 10, prompt: 'Record "Oven cleaning" demo in Hindi', type: 'audio', reward: 1.50, estTime: '45s', rarity: 'rare', category: 'kitchen' },
    { id: 11, prompt: 'Label floor types in 10 room photos', type: 'annotation', reward: 0.90, estTime: '55s', rarity: 'common', category: 'flooring' },
    { id: 12, prompt: 'Photograph dust buildup patterns', type: 'image', reward: 0.65, estTime: '40s', rarity: 'common', category: 'dusting' },
  ];

  const jobs: Job[] = [
    { id: 1, title: "Deep Clean - 4BR House", location: "West Leederville", pay: 180, urgency: "immediate", type: "Deep Clean", clientRating: 4.8, distance: "1.2km" },
    { id: 2, title: "Office Tower - 20 Floors", location: "Perth CBD", pay: 450, urgency: "soon", type: "Commercial", clientRating: 4.9, distance: "3.5km" },
    { id: 3, title: "End of Lease - 2BR", location: "Subiaco", pay: 120, urgency: "standard", type: "End of Lease", clientRating: 4.5, distance: "2.1km" },
    { id: 4, title: "Medical Centre - Daily", location: "Nedlands", pay: 95, urgency: "recurring", type: "Commercial", clientRating: 5.0, distance: "4.0km" },
    { id: 5, title: "Retail Space - Daily", location: "Claremont", pay: 75, urgency: "recurring", type: "Commercial", clientRating: 4.7, distance: "5.2km" },
    { id: 6, title: "Post-Reno Clean - Duplex", location: "Mount Lawley", pay: 200, urgency: "immediate", type: "Deep Clean", clientRating: 4.6, distance: "2.8km" },
  ];

  const offers: Offer[] = [
    { id: 1, client: "Perth CBD Properties", job: "Office Tower - Weekly", pay: 450, time: "Today 2PM" },
        ...
        <section className="py-4 bg-white dark:bg-zinc-800 shadow-md">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
            <div className="flex flex-col sm:flex-row justify-between items-center gap-4">
              <div>
                <p className="font-bold text-xl text-zinc-900 dark:text-white">Cleaner: Sarah Wilson</p>
          ...
          <div className="space-y-8">
            <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
              ...
              {badges.slice(0, 5).map((badge, i) => (
                <div key={i} className="bg-white dark:bg-zinc-800 p-4 rounded-xl shadow text-center">
                ...
                {tasks.map((task) => (
                  ...
                  >
                    <div className="flex justify-between items-start mb-2">
                      <span className={`px-2 py-1 rounded text-xs font-medium ${getRarityColor(task.rarity)}`}>
                        {task.rarity}
                      </span>
                ...
                {jobs.map((job) => (
                  <div key={job.id} className="p-4 bg-zinc-50 dark:bg-zinc-700 rounded-xl">
                    <div className="flex justify-between items-start mb-3">
                      <span className={`px-2 py-1 rounded text-xs text-white ${getUrgencyColor(job.urgency)}`}>
                        {job.urgency}
                      </span>
                      <span className="font-bold text-emerald-600">${job.pay}</span>
                    </div>
                    <h3 className="font-semibold text-zinc-900 dark:text-white mb-1">{job.title}</h3>
          ...
          <div className="space-y-6">
            <div className="bg-white dark:bg-zinc-800 rounded-2xl p-6 shadow-lg">
              ...
              <div className="space-y-4">
                {offers.map((offer) => (
                  <div key={offer.id} className="p-4 bg-zinc-50 dark:bg-zinc-700 rounded-xl flex justify-between items-center">
                    <div>
                      <h3 className="font-semibold text-zinc-900 dark:text-white">{offer.client}</h3>
            ...
            <div className="bg-white dark:bg-zinc-800 rounded-2xl p-6 shadow-lg">
              ...
              <div className="space-y-3">
                {leaderboard.map((entry) => (
                  <div key={entry.rank} className="p-4 bg-zinc-50 dark:bg-zinc-700 rounded-xl flex justify-between items-center">
                    <div className="flex items-center gap-4">
                      <span className="text-2xl">{entry.badge}</span>
                      <div>
                        <h3 className="font-semibold text-zinc-900 dark:text-white">{entry.name}</h3>
                        <p className="text-sm text-zinc-500">{entry.tasks} tasks</p>
                      </div>
                    </div>
                    <div className="text-right">
                      <p className="text-xl font-bold text-emerald-600">${entry.earnings.toFixed(2)}</p>
                    </div>
                  </div>
                ))}
              </div>
          ...
          <div className="bg-white dark:bg-zinc-800 rounded-2xl p-6 max-w-lg w-full max-h-[90vh] overflow-y-auto shadow-2xl">
            ...
            <div className="grid grid-cols-2 gap-4 mb-4 text-sm">
              <div>
                ...
                <span className="ml-2 font-bold text-emerald-600">${selectedTask.reward.toFixed(2)}</span>
              </div>
              <div>
              ...
              <div>
                ...
                <span className={`ml-2 px-2 py-0.5 rounded text-xs ${getRarityColor(selectedTask.rarity)}`}>
                  {selectedTask.rarity}
                </span>
              </div>
              <div>
            ...
            <div className="grid grid-cols-2 gap-4 mt-4 p-3 bg-zinc-50 dark:bg-zinc-700 rounded-lg text-sm">
              ...
              <div>
                ...
                <span className={`ml-2 font-medium ${aiScore === "Approved" ? "text-green-600" : "text-amber-600"}`}>
                  {aiScore}
                </span>
              </div>
              <div>
