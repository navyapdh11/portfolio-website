...
export default function ServicesTab({ services, editingService, setEditingService, updateService, deleteService }: ServicesTabProps) {
  return (
          ...
          <div key={service.id} className="bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6 hover:border-slate-600/50 transition-all">
            {editingService === service.id ? (
              ...
              <>
                ...
                <div className="flex gap-2">
                  <button onClick={() => setEditingService(service.id)} className="px-3 py-2 bg-sky-500/20 hover:bg-sky-500/30 text-sky-400 rounded-lg text-sm font-medium transition-colors">
                    ✏️ Edit
                  </button>
