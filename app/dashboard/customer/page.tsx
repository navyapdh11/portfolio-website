...
interface Customer {
  ...
  totalSpent: number;
  addresses: { street: string; suburb: string; state: string; postcode: string; isDefault: boolean }[];
  notifications: Notification[];
  reviews: Review[];
}

type TabType = "overview" | "bookings" | "payments" | "loyalty" | "reviews" | "notifications" | "profile" | "cart";
      ...
      <nav className="bg-slate-900/60 backdrop-blur-xl border-b border-slate-700/30 sticky top-[73px] z-40">
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex gap-1 overflow-x-auto py-2">
            {(["overview", "bookings", "payments", "loyalty", "reviews", "notifications", "profile", "cart"] as TabType[]).map(tab => (
              <button
                key={tab}
              ...
              >
                ...
                {tab === "notifications" && customer?.notifications?.filter(n => !n.read).length ? (
        ...
        {activeTab === "overview" && (
          <div className="space-y-8">
            ...
            <div className="grid grid-cols-2 md:grid-cols-4 gap-4">
              {[
                { label: "Total Bookings", value: customer?.totalBookings || 0, icon: "📅", color: "from-sky-500 to-blue-600" },
                { label: "Total Spent", value: `$${customer?.totalSpent || 0}`, icon: "💰", color: "from-green-500 to-emerald-600" },
                { label: "Loyalty Points", value: customer?.loyaltyPoints || 0, icon: "⭐", color: "from-amber-500 to-orange-600" },
                { label: "Reviews", value: customer?.reviews?.length || 0, icon: "📝", color: "from-purple-500 to-pink-600" },
              ].map((stat) => (
                <div key={stat.label} className="relative overflow-hidden bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6 group hover:border-slate-600/50 transition-all">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              <Link href="/#booking" className="bg-gradient-to-br from-sky-500/20 to-blue-600/20 backdrop-blur-xl border border-sky-500/30 rounded-2xl p-6 text-center hover:from-sky-500/30 hover:to-blue-600/30 transition-all group">
              ...
              <button onClick={() => setActiveTab("bookings")} className="bg-gradient-to-br from-green-500/20 to-emerald-600/20 backdrop-blur-xl border border-green-500/30 rounded-2xl p-6 text-center hover:from-green-500/30 hover:to-emerald-600/30 transition-all group">
                <div className="text-4xl mb-3">📋</div>
              ...
              <button onClick={() => setActiveTab("loyalty")} className="bg-gradient-to-br from-amber-500/20 to-orange-600/20 backdrop-blur-xl border border-amber-500/30 rounded-2xl p-6 text-center hover:from-amber-500/30 hover:to-orange-600/30 transition-all group">
                <div className="text-4xl mb-3">🎁</div>
            ...
            <div className="bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6">
              ...
              <div className="space-y-3">
                {bookings.slice(0, 5).map((booking) => (
                  <div key={booking.id} className="flex items-center justify-between p-4 bg-slate-700/50 rounded-xl">
                    <div className="flex items-center gap-4">
                      <div className="w-10 h-10 bg-sky-500/20 rounded-lg flex items-center justify-center text-lg">🧹</div>
                      <div>
        ...
        {activeTab === "bookings" && (
          <div className="space-y-6">
            <div className="flex justify-between items-center">
              ...
              <Link href="/#booking" className="px-6 py-3 bg-sky-500 hover:bg-sky-600 text-white rounded-xl font-medium transition-colors">
                + New Booking
              </Link>
            </div>
            <div className="grid gap-4">
              {bookings.map((booking) => (
                <div key={booking.id} className="bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6 hover:border-slate-600/50 transition-all">
                  <div className="flex flex-col md:flex-row md:items-center justify-between gap-4">
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-sm text-slate-400">{booking.address}, {booking.suburb}</p>
                        <p className="text-sm text-slate-400">{booking.date} at {booking.time}</p>
                      </div>
                    </div>
                    <div className="flex items-center gap-4">
          ...
          <div className="space-y-6">
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              ...
              <div className="bg-gradient-to-br from-blue-500/20 to-sky-600/20 backdrop-blur-xl border border-blue-500/30 rounded-2xl p-6">
                ...
                <p className="text-3xl font-bold text-white">${bookings.filter(b => b.status === "pending").reduce((s, b) => s + b.totalPrice, 0)}</p>
              </div>
              <div className="bg-gradient-to-br from-amber-500/20 to-orange-600/20 backdrop-blur-xl border border-amber-500/30 rounded-2xl p-6">
                ...
                <p className="text-3xl font-bold text-white">{bookings.length ? `$${Math.round(bookings.reduce((s, b) => s + b.totalPrice, 0) / bookings.length)}` : "$0"}</p>
              </div>
            </div>
            <div className="bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6">
              ...
              <div className="space-y-3">
                {bookings.filter(b => b.status === "completed").map((booking) => (
                  <div key={booking.id} className="flex justify-between items-center p-4 bg-slate-700/50 rounded-xl">
                    <div>
                      <p className="font-medium text-white">{booking.service}</p>
                    ...
                    <div className="flex items-center gap-4">
                      <p className="font-bold text-white">${booking.totalPrice}</p>
                      <span className="px-3 py-1 bg-green-500/20 text-green-400 rounded-full text-sm font-medium">Paid</span>
                    </div>
                  </div>
                ))}
              </div>
        ...
        {activeTab === "loyalty" && (
          <div className="space-y-6">
            <div className="bg-gradient-to-br from-amber-500/20 via-orange-500/20 to-red-500/20 backdrop-blur-xl border border-amber-500/30 rounded-2xl p-8 text-center">
              ...
              <h2 className="text-3xl font-bold text-white mb-2">{customer?.loyaltyPoints || 0} Points</h2>
              ...
              <p className="text-sm text-slate-400 mt-2">{customer?.loyaltyPoints || 0} / 1,000 to Gold tier</p>
            </div>

            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
              {[
                { points: 100, reward: "$10 Off Next Clean", icon: "💰", available: (customer?.loyaltyPoints || 0) >= 100 },
                { points: 250, reward: "Free Window Cleaning", icon: "🪟", available: (customer?.loyaltyPoints || 0) >= 250 },
                { points: 500, reward: "Free Deep Clean", icon: "✨", available: (customer?.loyaltyPoints || 0) >= 500 },
              ].map((reward) => (
                <div key={reward.points} className={`bg-slate-800/50 backdrop-blur-xl border rounded-2xl p-6 text-center ${reward.available ? "border-amber-500/30" : "border-slate-700/50 opacity-50"}`}>
            ...
            {customer?.reviews?.length ? (
              <div className="grid gap-4">
                {customer.reviews.map((review) => (
                  <div key={review.id} className="bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6">
                    <div className="flex items-center gap-2 mb-2">
                      {[...Array(5)].map((_, i) => (
                        <span key={i} className={`text-xl ${i < review.rating ? "text-amber-400" : "text-slate-600"}`}>★</span>
                      ))}
                    </div>
            ...
            <div className="space-y-3">
              {customer?.notifications?.length ? (
                customer.notifications.map((notif) => (
                  <div key={notif.id} className={`bg-slate-800/50 backdrop-blur-xl border rounded-2xl p-6 ${notif.read ? "border-slate-700/50" : "border-sky-500/30"}`}>
            ...
            <div className="bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                <div>
                  ...
                  <input type="text" defaultValue={customer?.name} className="w-full px-4 py-3 bg-slate-700/50 border border-slate-600 rounded-xl text-white focus:border-sky-500 focus:outline-none" />
                </div>
                <div>
                  <label className="block text-sm text-slate-400 mb-1">Email</label>
                  <input type="email" defaultValue={customer?.email} className="w-full px-4 py-3 bg-slate-700/50 border border-slate-600 rounded-xl text-white focus:border-sky-500 focus:outline-none" />
                </div>
                <div>
              ...
              <div className="space-y-3">
                {customer?.addresses?.map((addr, idx) => (
                  <div key={`${addr.postcode}-${idx}`} className="flex justify-between items-center p-4 bg-slate-700/50 rounded-xl">
                    ...
                    {addr.isDefault && <span className="px-3 py-1 bg-sky-500/20 text-sky-400 rounded-full text-sm">Default</span>}
                  </div>
                ))}
              </div>
            </div>
            ...
            {cart.length ? (
              <>
                <div className="space-y-4">
                  {cart.map((item) => (
                    <div key={item.id} className="bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-6 flex justify-between items-center">
                      <div>
                      ...
                      <div className="flex items-center gap-4">
                        ...
                        <button onClick={() => removeFromCart(item.id)} className="p-2 bg-red-500/20 hover:bg-red-500/30 text-red-400 rounded-lg transition-colors">
                          ✕
                        </button>
                      </div>
                    </div>
              ...
              <div className="bg-slate-800/50 backdrop-blur-xl border border-slate-700/50 rounded-2xl p-12 text-center">
                ...
                <Link href="/pricing" className="px-6 py-3 bg-sky-500 hover:bg-sky-600 text-white rounded-xl font-medium transition-colors">
                  Browse Services
                </Link>
              </div>
            )}
