...
export async function POST(request: Request) {
  ...
  try {
    ...
    return NextResponse.json({ success: true, service }, { status: 201, headers: rateLimitHeaders });
  } catch (err) {
...
export async function PATCH(request: Request) {
  ...
  try {
    ...
    if (!service) return NextResponse.json({ error: 'Not found' }, { status: 404, headers: rateLimitHeaders });
...
export async function DELETE(request: Request) {
  ...
  try {
    const body = await request.json();
    const { id } = body;
    if (!db.services.delete(id as string)) return NextResponse.json({ error: 'Not found' }, { status: 404, headers: rateLimitHeaders });
