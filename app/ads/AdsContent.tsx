...
export default function AdsContent() {
  const [activeTab, setActiveTab] = useState<"campaigns" | "competitor" | "seo" | "analytics">("campaigns");

  const campaigns: AdCampaign[] = [
    { id: 1, platform: "facebook", name: "Spring Clean Special", status: "active", budget: 500, spent: 312, impressions: 45000, clicks: 890, conversions: 23, ctr: 1.98, roas: 4.2 },
    { id: 2, platform: "instagram", name: "Before/After Gallery", status: "active", budget: 300, spent: 187, impressions: 28000, clicks: 560, conversions: 18, ctr: 2.0, roas: 3.8 },
    { id: 3, platform: "google", name: "Perth Cleaning Search", status: "active", budget: 800, spent: 524, impressions: 12000, clicks: 340, conversions: 45, ctr: 2.83, roas: 5.2 },
    { id: 4, platform: "facebook", name: "End of Lease Promo", status: "paused", budget: 400, spent: 145, impressions: 18000, clicks: 290, conversions: 12, ctr: 1.61, roas: 2.9 },
    { id: 5, platform: "google", name: "Commercial Cleaning", status: "active", budget: 600, spent: 398, impressions: 8500, clicks: 220, conversions: 31, ctr: 2.59, roas: 4.8 },
  ];

  const competitors: Competitor[] = [
    { id: 1, name: "CleanPro Perth", adspend: 4200, keywords: ["perth cleaning", "house cleaning perth", "commercial cleaning"], traffic: 8500, ranking: 1 },
    { id: 2, name: "Sparkle Services", adspend: 3100, keywords: ["cleaning services perth", "office cleaning"], traffic: 6200, ranking: 2 },
    { id: 3, name: "EcoClean WA", adspend: 2800, keywords: ["eco cleaning perth", "green cleaning"], traffic: 4800, ranking: 3 },
    { id: 4, name: "Perth Shine", adspend: 1900, keywords: ["carpet cleaning perth", "end of lease cleaning"], traffic: 3200, ranking: 4 },
  ];

  const seoMetrics: SeoMetric[] = [
    { keyword: "cleaning services perth", ranking: 12, volume: 2400, difficulty: 67, trend: "up" },
    { keyword: "house cleaning perth", ranking: 8, volume: 1800, difficulty: 58, trend: "up" },
    { keyword: "end of lease cleaning perth", ranking: 5, volume: 920, difficulty: 45, trend: "stable" },
    { keyword: "commercial cleaning perth", ranking: 15, volume: 1100, difficulty: 72, trend: "down" },
    { keyword: "deep cleaning perth", ranking: 3, volume: 650, difficulty: 38, trend: "up" },
  ];
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-8">
          ...
          <div className="bg-white dark:bg-zinc-800 p-5 rounded-xl shadow-md">
            ...
            <p className="text-2xl font-bold text-indigo-600">{(totalImpressions / 1000).toFixed(1)}K</p>
          </div>
        </div>
              ...
              <div className="space-y-4">
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="p-4 bg-red-50 dark:bg-red-900/20 rounded-xl">
                    <h3 className="font-semibold text-zinc-900 dark:text-white mb-4">🦴 Competitor Analysis</h3>
                    <div className="space-y-3">
                      {competitors.map((comp) => (
                        <div key={comp.id} className="flex justify-between items-center p-3 bg-white dark:bg-zinc-800 rounded-lg">
                          <div>
                            <p className="font-medium">#{comp.ranking} {comp.name}</p>
                            <p className="text-xs text-zinc-500">{comp.keywords.join(", ")}</p>
                          </div>
                          <div className="text-right">
                            <p className="font-bold text-red-600">${comp.adspend}/mo</p>
                            <p className="text-xs text-zinc-500">{comp.traffic.toLocaleString()} visits</p>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="p-4 bg-blue-50 dark:bg-blue-900/20 rounded-xl">
                    <h3 className="font-semibold text-zinc-900 dark:text-white mb-4">🎯 Keyword Opportunities</h3>
                    <div className="space-y-2">
                      {seoMetrics.map((metric) => (
                        <div key={metric.keyword} className="flex justify-between items-center p-2 bg-white dark:bg-zinc-800 rounded-lg text-sm">
                          <span className="text-zinc-700 dark:text-zinc-300">{metric.keyword}</span>
                          <span className={`font-medium ${getTrendColor(metric.trend)}`}>
                            {metric.trend === "up" ? "↑" : metric.trend === "down" ? "↓" : "→"} #{metric.ranking}
                          </span>
                        </div>
                      ))}
                    </div>
              ...
              <div className="space-y-6">
                <div className="grid md:grid-cols-2 gap-6">
                  <div className="p-4 bg-zinc-50 dark:bg-zinc-700 rounded-xl">
                    ...
                    <div className="space-y-3">
                      {[
                        ...
                        { label: "Conversions", value: totalConversions, pct: 100 },
                      ].map((step) => (
                        <div key={step.label}>
                          ...
                          <div className="h-3 bg-zinc-200 dark:bg-zinc-600 rounded-full overflow-hidden">
                            <div className="h-full bg-purple-500" style={{ width: `${step.pct}%` }}></div>
                          </div>
                        </div>
                      ))}
                    </div>
                  ...
                  <div className="p-4 bg-zinc-50 dark:bg-zinc-700 rounded-xl">
                    ...
                    <div className="space-y-3">
                      {[
                        { source: "Google Ads", value: 65, color: "bg-green-500" },
