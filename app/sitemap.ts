...
import { allSuburbs, suburbsByState } from "@/lib/data/suburbs";
...
export default async function sitemap(): Promise<MetadataRoute.Sitemap> {
	...
	const staticPages: MetadataRoute.Sitemap = [
		...
		{ url: `${BASE_URL}/services`, lastModified: new Date(), changeFrequency: "weekly", priority: 0.85 },
	...
	const serviceCityPages: MetadataRoute.Sitemap = cleaningServices.flatMap((service) =>
		[
			{ slug: "sydney", state: "nsw" }, { slug: "melbourne", state: "vic" },
			{ slug: "brisbane", state: "qld" }, { slug: "perth", state: "wa" },
			{ slug: "adelaide", state: "sa" }, { slug: "hobart", state: "tas" },
			{ slug: "canberra", state: "act" }, { slug: "darwin", state: "nt" },
		].map((city) => ({
	...
	console.log(`   Service-Suburb (top): ${serviceSuburbPages.length}, States: ${statePages.length}`);
